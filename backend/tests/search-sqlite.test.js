// tests/search-sqlite.test.js — Day search on SQLite backend:
// task-title matching + AND semantics (mirror of search.test.js, DB_TYPE='sqlite')
const path = require('path');
const fs = require('fs');
const os = require('os');

// 必须在加载数据库模块前设置测试环境变量与数据目录
const testDataDir = path.join(os.tmpdir(), 'timetrace-search-sqlite-test-' + Date.now());
...
describe('Days API — search by keyword on SQLite (2.0 task-title matching)', () => {
  const uniqueEmail = () => `search-sqlite-${Date.now()}-${emailCounter}-${Math.random().toString(36).slice(2)}@example.com`;

  const register = async () => {
    const res = await request(app)
      .post('/api/auth/register')
      .send({ username: `searchsqlite${emailCounter}`, email: uniqueEmail(), password: 'password123' })
      .expect(201);
  ...
  test('two keywords use AND semantics within a single task title', async () => {
    const token = await register();
